 ...
 * @description - it gets the values from the input fields, decodes the code that the user inputs, 
 * and changes the CSS variables based on the decoded values.
 */
function genCode() {
    bg.value = '';
    theme.value = '';
    ...
    txt2.value = txt2c;
    bx.value = bxc;
    r.style.setProperty('--background', bgc);
...
new lc_color_picker('input[name="background"]', {
    transparency: true,
    open_on_focus: true,
    wrap_width: '100%',
    preview_style: {
    ...
    on_change: function (new_value, target_field) {
        r.style.setProperty('--background', new_value);
    },
});
new lc_color_picker('input[name="theme"]', {
    ...
    on_change: function (new_value, target_field) {
        r.style.setProperty('--theme', new_value);
    },
});
new lc_color_picker('input[name="theme-secondary"]', {
    ...
    on_change: function (new_value, target_field) {
        r.style.setProperty('--theme-secondary', new_value);
    },
});
new lc_color_picker('input[name="text"]', {
    ...
    on_change: function (new_value, target_field) {
        r.style.setProperty('--text', new_value);
    },
});
new lc_color_picker('input[name="text-secondary"]', {
    ...
    on_change: function (new_value, target_field) {
        r.style.setProperty('--text-secondary', new_value);
    },
});
new lc_color_picker('input[name="box"]', {
    ...
    on_change: function (new_value, target_field) {
        r.style.setProperty('--box', new_value);
    },
});
